...
function derivatives are handled with the `derivative` method.
```python
x = var(3)
f = lambda x: 3*x+4
...
└── README.md
```
 - `autodiff_107` contains `diff`, `optim` and `viz` subdirectories. `diff` contains Node.py, which contain Node, our main class for variables. nodemath.py deals with overriding numpy functions for members of Node. `optim` has optimization functions in optimize.py and rootfinding algorithms in rootfinding.py. `viz` contains the code for building the vizualisation of the computational graphs.
...
<p align="justify">Right now only gradient descent and adagrad are implemented. We could also implement further gradient descent algorithms such as SGD (Stochastic Gradient Descent), ADAM, RMS Prop etc.

## Training an Artificial Neural Network (ANN) using our package

<p align="justify">With our optimization algortihms we could train a neural network using our package. This will probably be a very small network, trained on MNIST. We will thus also need to implement the sigmoid function and the rely function.

## Future direction for visualisation

<p align="justify">We can also try to make nice visualisation as gifs. Furthermore, we could also create a table to show the independent and dependent variables along the tangent trace, as we have done in class. </p>
